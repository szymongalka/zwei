...
def _ctx() -> CommandContext:
    msg = InboundMessage(
        channel="telegram", sender_id="user", chat_id="direct", content="/model-menu"
    )
    return CommandContext(
        msg=msg, session=None, key=msg.session_key, raw="/model-menu", args="", loop=MagicMock()
    )
...
def test_model_menu_is_dispatchable() -> None:
    ...
    assert router.is_dispatchable_command("/model-menu")


def test_model_menu_listed_in_help() -> None:
    assert "/model-menu" in build_help_text()
