...
class PersonalStore:
    ...
    def put(self, source: str, key: str, payload: object, text: str | None = None) -> str:
        with self.db() as db:
            return self._put(db, source, key, payload, text)
    ...
    def search(self, query: str, limit: int = 10) -> list[dict[str, Any]]:
        ...
        with self.db() as db:
            rows = db.execute("""
                SELECT d.id,d.source,d.item_key,d.text,d.created
    ...
    def pending(self, limit: int = 50) -> list[dict[str, Any]]:
        with self.db() as db:
            return [dict(row) for row in db.execute("""
                SELECT d.* FROM documents d JOIN outbox o ON d.id=o.id
